...
/* SCM stubs for functions absent from the upstream qcom_scm driver.
 *
 ...
 */
#if defined(CONFIG_QCOM_KGSL_UPSTREAM) && !IS_ENABLED(CONFIG_QCOM_SCM)
...
static inline int qcom_scm_kgsl_set_smmu_lpac_aperture(unsigned int num_context_bank)
{
	return -EINVAL;
}
static inline int qcom_scm_set_gpu_smmu_aperture(unsigned int num_context_bank)
{
	return -EINVAL;
}
