...
- `configs/oiw_camera_media.config` — the same OIW kernel config fragment as
  `noods1234/xiaomi_13pro_sukisu-ultra_aosp`'s `kernel/oiw/nuwa/configs/oiw_camera_media.config` (kept
...
2. Merge `oiw/nuwa/configs/oiw_camera_media.config` into that branch's defconfig and confirm it still
